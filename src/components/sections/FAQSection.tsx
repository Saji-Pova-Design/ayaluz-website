...
export default function FAQSection({ data }: Props) {
  ...
  const firstTabId = categories[0]?._key || categories[0]?.id || "faq-tab-0";
  ...
  const activeCategory = categories.find((category, index) => {
    const tabId = category._key || category.id || `faq-tab-${index}`;
            ...
            <div className="flex min-w-max gap-8 px-1 md:gap-12">
              {categories.map((category, index) => {
                ...
                return (
                  ...
                  >
                    <span className={isActive ? "text-[#2B4A40]" : ""}>
                      {category.label}
                    </span>
        ...
        {activeCategory && (
          ...
          >
            {(activeCategory.questions || []).map((item, index) => {
              ...
              const richAnswer = item.answerRichText;
              const fallbackAnswer = item.answer;

              return (
                <div key={itemKey} className="border-b border-[#2B4A40]/15">
                  <button
                  ...
                  >
                    ...
                    >
                      <ChevronDown size={28} strokeWidth={1.6} />
                    </motion.span>
                  </button>
                    ...
                    {isOpen && (
                      ...
                      >
                        <div className="max-w-[820px] pb-7 pr-8">
                          {Array.isArray(richAnswer) && richAnswer.length > 0 ? (
                          ...
                          ) : Array.isArray(fallbackAnswer) && fallbackAnswer.length > 0 ? (
                            <div className="space-y-4">
                              <PortableText
