...
export function shareViaInstagram(): ShareActionResult {
  const isMobile =
  ...
  return {
    status: "unavailable",
    message:
      "Instagram sharing is not available here — copy the link and paste it in your story or DM.",
  };
}
...
export async function tryNativeShare(
  ...
  if (
    ...
    return {
      status: "unavailable",
      message: "Native share not supported",
    };
  }
...
export function getShareDomain(): string {
  try {
    return new URL(getSiteUrl()).hostname.replace(
      /^www\./,
      "",
    );
  } catch {
